...
func getInterchainAccountCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:  "account [account] [connection-id]",
...
func getInterchainAccountsCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:  "accounts [connection-id]",
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			queryClient := types.NewQueryClient(clientCtx)
			res, err := queryClient.InterchainAccountsWithConnection(cmd.Context(), &types.QueryInterchainAccountsWithConnectionRequest{ConnectionId: args[0]})
			if err != nil {
				return err
			}
